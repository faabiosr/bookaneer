...
// qpRow is a scan-only struct for reading quality_profiles rows via sqlx.
// Items is stored as JSON text in the DB and unmarshalled after scanning.
type qpRow struct {
	ID     int64  `db:"id"`
	Name   string `db:"name"`
...
func (r qpRow) toProfile() QualityProfile {
	return QualityProfile{
		ID:     r.ID,
		Name:   r.Name,
		Cutoff: r.Cutoff,
		Items:  UnmarshalItems(r.Items),
	}
}
