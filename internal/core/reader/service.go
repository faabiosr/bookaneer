...
func (s *Service) GetBookFile(ctx context.Context, id int64) (*BookFile, error) {
	...
	err := s.db.GetContext(ctx, &bf, `
		SELECT bf.id, bf.book_id, bf.edition_id, bf.path, bf.relative_path,
		       ...
		       b.title AS book_title, a.name AS author_name, COALESCE(b.image_url, '') AS cover_url
		FROM book_files bf
...
func (s *Service) ListBookFiles(ctx context.Context, bookID int64) ([]BookFile, error) {
	...
	if err := s.db.SelectContext(ctx, &files, `
		SELECT id, book_id, edition_id, path, relative_path,
		       size, format, quality, hash, added_at
		FROM book_files
		WHERE book_id = ?
